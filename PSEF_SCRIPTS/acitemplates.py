...
# EPGs attachment
    ...
    for epg_cons_el in epg_dict_cons:
        if (config_epg_cons == ''):
            config_epg_cons = aci_attach_epg(name, epg_cons_el['address-set-alias-2'], epg_cons_el['acipath'], 'cons') 
        else:
            config_epg_cons = config_epg_cons + '\n' + aci_attach_epg(name, epg_cons_el['address-set-alias-2'], epg_cons_el['acipath'], 'cons')
    for epg_prov_el in epg_dict_prov:
        if (config_epg_prov == ''):
            config_epg_prov = aci_attach_epg(name, epg_prov_el['address-set-alias-2'], epg_prov_el['acipath'], 'prov')        
        else:
            config_epg_prov = config_epg_prov + '\n' + aci_attach_epg(name, epg_prov_el['address-set-alias-2'], epg_prov_el['acipath'], 'prov')
...
# EPGs attachment
    ...
    for epg_cons_el in epg_dict_cons:
        if (config_epg_cons == ''):
            config_epg_cons = aci_detach_epg(name, epg_cons_el['address-set-alias-2'], epg_cons_el['acipath'], 'cons')
        else:
            config_epg_cons = config_epg_cons + '\n' + aci_detach_epg(name, epg_cons_el['address-set-alias-2'], epg_cons_el['acipath'], 'cons')
    for epg_prov_el in epg_dict_prov:
        if (config_epg_prov == ''):
            config_epg_prov = aci_detach_epg(name, epg_prov_el['address-set-alias-2'], epg_prov_el['acipath'], 'prov')
        else:
            config_epg_prov = config_epg_prov + '\n' + aci_detach_epg(name, epg_prov_el['address-set-alias-2'], epg_prov_el['acipath'], 'prov')
